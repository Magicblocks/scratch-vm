...
const ArduinoDigitalIn = ['Pulsante A', 'Pulsante B', 'Pulsante C', 'Pulsante D' ];
const ArduinoAnalogIn = ['Potenziometro','Sensore Luce'];
...
class Scratch3ArduinoBlocks {
			...
			blocks: [
				...
				*/
				
				{
					...
					blockType: BlockType.BOOLEAN,
					text: 'Se [DIGITAL_IN] = [BOOL_VAL]',
					
					arguments: {
						DIGITAL_IN: {
							type: ArgumentType.STRING,
							menu: 'mDigitalIn',
							defaultValue: ArduinoDigitalIn[0]
						},
						BOOL_VAL: {
							type: ArgumentType.STRING,
							menu: 'mBoolVal',
	...
	setAnimation(args){
		...
		return this._sendWsData(SET_ANIM, anim_num, 0x00);
	}
	...
	ifDigitalRead(args){
		...
		let pinName = args.DIGITAL_IN;		
		let pin = ArduinoDigitalIn.indexOf(pinName);
		...
		if (args.BOOL_VAL == ArduinoBoolValues[0] && pin != 4)
			state = !state;
